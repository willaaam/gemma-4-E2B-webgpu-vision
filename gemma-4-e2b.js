...
queueMicrotask(() => { if (typeof Mt !== "function") return; const load = Mt.load; Mt.load = async function(...args) { const model = await load.apply(this, args); if (typeof model.countPromptTokens !== "function") model.countPromptTokens = messages => model.encodePrompt(messages).length; return model; }; });
queueMicrotask(() => { if (typeof Mt !== "function") return; const load = Mt.load; Mt.load = async function(...args) { const model = await load.apply(this, args); const empty = model.countPromptTokens([{ role: "user", content: "" }]); model.countTextTokens = text => Math.max(0, model.countPromptTokens([{ role: "user", content: String(text ?? "") }]) - empty); return model; }; queueMicrotask(() => { if (typeof Mt !== "function") return; const capabilityLoad = Mt.load; Mt.load = async function(...args) { const model = await capabilityLoad.apply(this, args); const read = model.getContextCapabilities?.bind(model); if (!read) return model; model.getContextCapabilities = () => { const capabilities = read(); const capacity = Number(model._generationState?.cache?.maxLength) || Number(capabilities?.currentCapacity) || 0; if (!capacity) return capabilities; const architecturalMax = Number(capabilities?.architecturalMax) || 131072; return { ...capabilities, effectiveContextMax: Math.min(capacity, architecturalMax), currentCapacity: capacity, allocation: { ...capabilities.allocation, status: capacity >= architecturalMax ? "validated" : (capabilities.allocation?.status === "reduced" ? "reduced" : "partial"), allocatedCapacity: capacity } }; }; return model; }; }); });
queueMicrotask(() => { if (typeof Mt !== "function") return; const architecturalMax = 131072, install = model => { const state = model._generationState, runtime = model.runtime, config = model._model?.config, originalGenerate = model.generate.bind(model), originalEncode = model.encodePrompt.bind(model); if (!state?.cache || !runtime || !config) return model; const cache = () => state.cache, deviceLimits = () => { const limits = runtime.device?.limits ?? runtime.host?.device?.limits ?? {}; return { maxBufferSize: Number(limits.maxBufferSize) || 0, maxStorageBufferBindingSize: Number(limits.maxStorageBufferBindingSize) || 0, maxComputeWorkgroupsPerDimension: Number(limits.maxComputeWorkgroupsPerDimension) || 65535 }; }, ensure = async required => { const old = cache(); if (required <= old.maxLength) return old.maxLength; if (required > config.max_position_embeddings) throw Object.assign(new Error(`Context request ${required} exceeds architectural maximum ${config.max_position_embeddings}`), { code: "context_limit", required, architecturalMax: config.max_position_embeddings, effectiveContextMax: old.maxLength }); const target = Math.min(config.max_position_embeddings, Math.max(required, Math.max(old.maxLength * 2, old.maxLength + 1024))); let next; try { next = model._model.createGenerationState({ maxLength: target }).cache; await runtime.queueIdle?.(); for (let i = 0; i < old.keys.length; i++) { const sourceK = old.keys[i], sourceV = old.values[i], targetK = next.keys[i], targetV = next.values[i]; if (!sourceK || !sourceV || !targetK || !targetV) continue; const layer = config.layers[i]; const bytes = old.seqLength * layer.kvOut * 4; await runtime.copyBufferToBuffer({ src: sourceK, dst: targetK, byteLength: bytes }); await runtime.copyBufferToBuffer({ src: sourceV, dst: targetV, byteLength: bytes }); } await runtime.queueIdle?.(); next.seqLength = old.seqLength; state.cache = next; old.dispose(); return target; } catch (error) { try { next?.dispose?.(); } catch (_) {} cache().allocationReason = String(error?.message ?? error); throw Object.assign(error, { code: "context_capacity", requested: required, effectiveContextMax: old.maxLength }); } }, capabilities = () => { const current = cache(); const limits = deviceLimits(); const layers = (config.layers ?? []).map(layer => ({ index: layer.index, cacheType: layer.isSliding ? "sliding" : "full", slidingWindow: layer.slidingWindow || 0, kvOut: layer.kvOut, kvSourceLayer: layer.kvSourceLayer, isKvShared: !!layer.isKvShared, capacity: current.maxLength })); const reason = current.allocationReason ?? null; return { architecturalMax: config.max_position_embeddings ?? architecturalMax, effectiveContextMax: reason ? current.maxLength : (config.max_position_embeddings ?? architecturalMax), initialCapacity: current.initialCapacity ?? current.maxLength, currentCapacity: current.maxLength, layers, cacheDtype: "float32", device: limits, prefill: { chunkSize: 2048, runnerBlockSizes: [32, 128, 256], maxComputeWorkgroupsPerDimension: limits.maxComputeWorkgroupsPerDimension }, allocation: { status: reason ? "reduced" : "lazy", requestedCapacity: current.maxLength, allocatedCapacity: current.maxLength, reason } }; }; model.countPromptTokens = messages => originalEncode(messages).length; model.getContextCapabilities = capabilities; model.ensureContextCapacity = ensure; model.generate = async function* (messages, options = {}) { const requested = Math.max(1, Number(options.maxNewTokens ?? 512) || 512); const promptTokens = originalEncode(messages).length; await ensure(Math.min(config.max_position_embeddings, promptTokens + requested + 1)); yield* originalGenerate(messages, options); }; return model; }; const originalLoad = Mt.load; Mt.load = async function(...args) { return install(await originalLoad.apply(this, args)); }; });
